...
/**currently does nothing... this code should allow the armor to be dyed as the Leather has been replaced with Chained Leather*/

public class DyeableArmorRecipes extends IForgeRegistryEntry.Impl<IRecipe> implements IRecipe {

    /**
     * Used to check if a recipe matches current recipes inventory
     */
    public boolean matches(InventoryCrafting inv, World worldIn)
    {
        ...
        for (int i = 0; i < inv.getSizeInventory(); ++i)
        {
            ...
            {
                if (itemstack1.getItem() instanceof DyeableArmor)
                {
                    ItemArmor itemarmor = (ItemArmor)itemstack1.getItem();

                    if (itemarmor.getArmorMaterial() != CHAINED_LEATHER || !itemstack.isEmpty())
                    {
                        return false;
                    }
